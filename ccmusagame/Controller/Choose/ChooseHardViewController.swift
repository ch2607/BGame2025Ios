...
class ChooseHardViewController: UIViewController {
...
extension ChooseHardViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout
{
               ...
               if myArray2[indexPath.row] == 1
               {
                ...
                else
                {
                    if myArray2[indexPath.row] == 0
                    {
                        ...
                        cell.imageView.image = UIImage(systemName: "lock.square.fill")
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        print("carl select = \(indexPath.row)")
            ...
            GameName.shared.ChooseLevel = 2
            GameName.shared.mname = "cmed"
            GameName.shared.mnum = indexPath.row + 1
            print("Carl indexpath.row = \(indexPath.row)")
            if myArray2[indexPath.row] == 1
            {
                let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "Choose") as! GameChooseViewController
                 self.present(vc, animated: true, completion: nil)
            }
